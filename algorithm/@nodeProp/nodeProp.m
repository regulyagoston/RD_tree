...
classdef nodeProp 
    ...
    %% Nonparametric estimation values - structure
    properties
       np_struct    % Nonparametric estimation tools 
    end
    ...
    methods
        ...
        function obj = updateNode( obj , data , optTree , tr )
            ...
            if strcmp( optTree.type , 'CATE' )
                ...
                elseif strcmp( optTree.model , 'local' )
                    obj.est_struct = est_treat_lbl_local( obj , data , optTree , tr );
                else
                    msg1 = true;
                end
        ...
        function obj = predictNode( obj , obj_tr , data , optTree , tr )
            ...
            %% If Leaf-by-leaf criterion -> estimate the relevant values
                ...
                % CATE estimations
                if strcmp( optTree.type , 'CATE' )
                    if any( strcmp( optTree.model , {'mean','linear','local'} ) )
    ...
    methods
        ...
        function tf = issameNode( node1 , node2 , tolerance )
            ...
            if ( isempty( node2 ) )
                return;
            elseif all( node1.subsetIdx == node2.subsetIdx ) && ...
