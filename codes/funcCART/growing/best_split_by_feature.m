...
elseif nZ > optTree.numSplit && ... 
       any( strcmp( optTree.splitType , {'causal-tree-AI','causal-tree-RDD'} ) )
    ...
    bf = true;
    [ bS_f , valid_split , ~ , split ] = createSplits( Zi , Wi , node , tr_tree , id_f , parentID , data , optTree );
...
if rel_ios <= optTree.cp%( 1 - rel_ios ) <= penalty_ios
    bS_f                      = node;
    bS_f.Failed               = 1;
    bS_f.feature_chck( id_f ) = false;
