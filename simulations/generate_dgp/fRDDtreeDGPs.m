%% Get Monte-Carlo simulation variables for fuzzy `tree' from Reguly (2025)
%
% Input:
%   type:
%   Calonico et al.:
...
function [ Y , Z , X , W , c , chi , eta , predValues , trueTree ] = fRDDtreeDGPs( type , M , N , sigma2_eps , seed )
...
%% Calonico et al simulations        
    case {'rdrobust-M1-2tr','rdrobust-M2-cntstr','rdrobust-M3-1tr'}
        ...
        X = X( 1 : N );
        c = 0;
        % For simplicity let the treatment equation be simple, one sided
        % homogeneous
        u = rand( [ N , 1 ] ); 
        chiW = ( X >= c ) * 10;
        etaW =  2 .* X - 6 .* X.^2 + 3 .* X .^3;
        Wprob = normcdf( etaW + chiW );
        W = u < Wprob;
        ...
        elseif strcmp( type , 'rdrobust-M2-cntstr' )
            ...
            %chi  = ( 0.26 - 3.71 )
            %chi  = -5.45 - ( Z_age - 5 );
            chi = -0.45 + 0.5*Z_age - 0.25*Z_age.^2 + 0.1*Z_age.^3;
